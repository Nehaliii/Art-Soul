...
{    
    ...
    if(empty($name) || empty($phone_no) || empty($email) || empty($address)) {            
        ...
        //updating the table
        $result = mysqli_query($mysqli, "UPDATE artist SET artist_name='$name',phone_no='$phone_no',email_id='$email',address='$address' WHERE id=$id");
...
if (isset($_GET['id'])) {
		$id = $_GET['id'];
		$update = true;
		$record = mysqli_query($mysqli, "SELECT * FROM artist WHERE id=$id");
...
?>
    ...
    <form name="form1" method="post" action="editartist.php">
        <table border="0" bgcolor='#CCCCCC'>
            <tr> 
                <th>Name</th>
                <td><input type="text" name="artist_name" value="<?php echo $name;?>"></td>
            </tr>
            <tr> 
                <th>Phone No</th>
